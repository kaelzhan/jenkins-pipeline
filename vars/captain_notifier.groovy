...
def captain_callback_onstart(){
    ...
    withEnv(["captain_json=${captain_json}"]){
        sh '''#!/bin/bash
        set +e
        curl --max-time 60 --insecure -k -f -H "Content-Type:application/json" -X POST -d "${captain_json}" http://jenkins:lko34kd9fd2@127.0.0.1:8000/app_dev.php/api/jenkins/callback
        if (( $? != 0 )); then
